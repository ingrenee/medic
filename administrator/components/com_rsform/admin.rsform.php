...
function formsEdit($formId)
{
    ...
    {
		$db->setQuery("INSERT INTO #__rsform_forms SET `FormName`='"._RSFORM_BACKEND_FORMS_EDIT_NO_FORM_NAME."', `FormTitle`='"._RSFORM_BACKEND_FORMS_EDIT_NO_FORM_TITLE."', `FormLayout`='', `FormLayoutName`='inline', `FormLayoutAutogenerate`='1'");
		$db->query();
		$formId = $db->insertid();
...
function componentsDisplay($option)
{
	...
	foreach ($fields as $r)
	{
		...
		switch($r['FieldType'])
		{
			...
			{
				$out .= '<td>'.(defined('_RSFORM_BACKEND_COMP_FIELD_'.$r['FieldName']) ? constant('_RSFORM_BACKEND_COMP_FIELD_'.$r['FieldName']):JText::_('_RSFORM_BACKEND_COMP_FIELD_'.$r['FieldName'])).'<br/>';
				$out .= '<select name="param['.$r['FieldName'].']" id="'.$r['FieldName'].'" >';
