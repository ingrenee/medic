 ...
 * @version     $Id: shmobile.php 1548 2010-08-22 09:04:40Z silianacom-svn $
 */
...
class  plgSystemShmobile extends JPlugin {
  ...
  public function onAfterRoute() {
    ...
    if ($enabled) {
      ...
      //print_r($browser);
      //die();
      $userAgent = $browser->get('_agent');
      // detection code adapted from http://detectmobilebrowser.com/
      $isMobile = $isMobile || preg_match('/android|avantgo|blackberry|blazer|compal|elaine|fennec|hiptop|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|mobile|o2|opera m(ob|in)i|palm( os)?|p(ixi|re)\/|plucker|pocket|psp|smartphone|symbian|treo|up\.(browser|link)|vodafone|wap|windows ce; (iemobile|ppc)|xiino/i',$userAgent)
