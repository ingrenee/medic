 ...
 * @version     $Id: sh404sef.class.php 1655 2010-10-11 15:43:07Z silianacom-svn $
 */
...
function shAddSefUrlToDBAndCache( $nonSefUrl, $sefString, $rank, $urlType) {
  ...
  if ($urlType == sh404SEF_URLTYPE_AUTO) {  // before adding a full sef, we must check it does not already exists as a 404
    ...
    if (!empty($result)) {
      ...
      $query = 'select id, newurl, rank from #__redirection where oldurl=\''.$sefString.'\' AND newurl <> \''.addslashes(urldecode($nonSefUrl)).'\' order by ' . $database->nameQuote( 'rank') . ' desc';
      $database->setQuery($query);
...
function shRawUrlDecodeDeep( $data) {
  ...
  if (is_array($data)) {
    return rawurldecode( array_shift($data));
  } else {
